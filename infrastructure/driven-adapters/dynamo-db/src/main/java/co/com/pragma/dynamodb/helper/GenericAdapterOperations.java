...
public abstract class GenericAdapterOperations<E, K, V> {
  ...
  protected GenericAdapterOperations(DynamoDbEnhancedAsyncClient dynamoDbEnhancedAsyncClient, ObjectMapper mapper, Function<V, E> toEntityFn, String tableName,
                                     String... index) {
  ...
  public Mono<E> getById(K id) {
    return Mono.fromFuture(table.getItem(Key.builder().partitionValue(AttributeValue.builder().n(String.valueOf(id)).build()).build())).map(this::toModel);
  }
